...
// ⭐ [주의] 구글 앱스 스크립트에서 '새 배포' 후 받은 최신 URL을 아래에 넣으세요.
const GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbzSQAZDoPgyO7uWr1seIFxw8szsAK6qKQbZOQf5KY9VXSFpe1tYp2KSAzszDQgtd1Zq/exec"; 
...
window.login = function () {
  ...
  fetch(GOOGLE_SCRIPT_URL + "?phone=" + inputVal)
    ...
    .then(data => {
      ...
      showCustomModal("접속 오류가 발생했습니다. 주소를 확인해주세요.");
...
function renderUnitButtons() {
  ...
  for (let i = 1; i <= 8; i++) {
    ...
    const titleText = currentTitles[i] ? `<br><span style="font-size:12px; font-weight:normal; color:#000;">${currentTitles[i]}</span>` : "";
...
// ----------------------
// 4. 학습 모드 (Script / Voca / 효과음 / 흔들림)
// ----------------------
window.startScriptMode = async function() { 
  currentPart = "Script"; currentTotalCycles = 18; loadStudyData(`${currentType}u${currentUnit}.json`); 
};

window.startVocaMode = async function() { 
  currentPart = "Voca"; currentTotalCycles = 10; loadStudyData(`${currentType}u${currentUnit}_voca.json`); 
};
...
async function loadStudyData(fileName) {
  ...
  try {
    ...
    updateProgress();
    showBox('study-box');
  } catch (e) { showCustomModal("파일 로드 실패"); showMenu(); }
}
...
function playSentence() {
  ...
  sText.classList.remove("shake"); // 이전 흔들림 제거
...
recognizer.onresult = (event) => {
  ...
  if (spoken.includes(target) || target.includes(spoken)) {
    successSound.play();
    sText.innerText = "Excellent!"; sText.style.color = "#39ff14";
...
window.nextStep = function() {
  ...
  sendDataToGoogle(currentPart, percent + "%"); // 구글 시트에 진행률 및 시간 저장
  
  if (percent >= 100 && !isAlertShown) { 
    isAlertShown = true; 
    triggerFireworkConfetti(); 
    showCustomModal(`${currentPart} 학습 100% 달성! 🎉`, () => playSentence()); 
    return; 
  }
  playSentence();
};
...
window.finishASStudy = function() {
  ...
  sendDataToGoogle("AS Correction", timeStr); // 학습 시간 시트 저장
...
window.submitAccurateSpeaking = async function() {
  ...
  try {
    ...
    if (data.result === "success") {
      showCustomModal("성공적으로 제출되었습니다! 🎉", () => showMenu()); // 폭죽 제거
    } else { throw new Error(data.message); }
  } catch (e) { showCustomModal("전송 실패: 서버 연결을 확인하세요."); showBox('as-record-box'); }
};

// ----------------------
// 7. Listen & Repeat (사이클 제어 및 스크롤)
// ----------------------
window.startRepeatMode = async function() {
  try {
    ...
    currentData.forEach((item, idx) => {
      ...
      div.style.cssText = "padding:10px; border-bottom:1px solid #222; text-align:left;";
...
window.showResultsPage = async function() {
  ...
  try {
    ...
    showBox('results-box');
  } catch (e) { showCustomModal("리포트 로드 실패"); }
};
